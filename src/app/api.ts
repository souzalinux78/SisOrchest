...
export const api = {
  ...
  downloadReportsPdf: async (params: {
    common_id: number
    month: number
    year: number
    weekday?: string | null
    ...
    const response = await fetch(`${API_BASE_URL}/reports/pdf?${queryParams.toString()}`, {
      method: 'GET',
      headers,
    })
    ...
    if (!response.ok) {
      ...
      const data = await response.json().catch(() => ({}))
      const message = data?.message ?? 'Falha ao exportar PDF.'
      throw new Error(message)
    }
